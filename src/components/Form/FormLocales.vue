<template>
	<div v-if="locales.length" class="pkpFormLocales">
		<button v-for="locale in additionalLocales"
			...
			:key="locale.key"
			:label="locale.label"
			:class="{'pkpFormLocales__locale--isActive': active.includes(locale.key)}"
...
export default {
	...
	computed: {
		...
		primaryLocale: function () {
			return this.locales.find((locale) => locale.key === $.pkp.app.primaryLocale);
		},
		...
		additionalLocales: function () {
			return this.locales.filter((locale) => locale.key !== $.pkp.app.primaryLocale);
		},
	},
	methods: {
		/**
		...
		toggleLocale: function (localeKey) {
			let selected = [this.primaryLocale.key];
